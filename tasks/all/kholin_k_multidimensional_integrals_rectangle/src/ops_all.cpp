...
bool kholin_k_multidimensional_integrals_rectangle_all::TestTaskALL::PostProcessingImpl() {
  ...
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  if (rank == 0) {
    reinterpret_cast<double*>(task_data->outputs[0])[0] = I_2n;
  }
  return true;
}

kholin_k_multidimensional_integrals_rectangle_all::TestTaskALL::~TestTaskALL() { MPI_Type_free(&mpi_size_t); }

MPI_Datatype kholin_k_multidimensional_integrals_rectangle_all::TestTaskALL::get_mpi_type() {
